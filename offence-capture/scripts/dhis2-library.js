...
iroad2 = {
		...
		Init : function(config){
			...
			http.get(iroad2.config.baseUrl + "api/dataElements?paging=false&fields=id,name,type,optionSet[id,name,code,options[id,name]]", function(results) {
				//Set the dhis data elements
				iroad2.data.dataElements = results.dataElements;
				//Fetch programs from the dhis server
				http.get(iroad2.config.baseUrl + "api/programs?paging=false", function(results2) {
 ...
 *	
 *	@param string modalName (Name of the Program in iroad2 to be mirrored)
 *
...
iroad2.data.Modal = function (modalName,relations) {
	...
	this.renderToJSON = function(event, onSuccess) {
		 ...
		 * 
		 * @param iroad2.data.Modal programModal
		 * 
		 * @param string id
		 */
	...
	this.save = function(data,otherData,onSuccess,onError){
		...
		http.post(iroad2.config.baseUrl + "api/events",JSON.stringify(sendData),function(results){
			onSuccess(results);
		},function(results){
			onError(results);
		});
